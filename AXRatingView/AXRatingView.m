...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat selfWidth;
    if (_distributeSpacingEvenly) {
        selfWidth = CGRectGetWidth(self.frame);
    } else {
        selfWidth = (self.markImage.size.width * _numberOfStar);
    }
...
- (CALayer *)generateMaskLayer
{
    ...
    CGFloat markWidth;
    if (_distributeSpacingEvenly) {
        markWidth = CGRectGetWidth(self.frame)/_numberOfStar;
    } else {
        markWidth = _markImage.size.width;
    }
    ...
    CGFloat totalStartsWidth;
    if (_distributeSpacingEvenly) {
        totalStartsWidth = CGRectGetWidth(self.frame);
    } else {
        totalStartsWidth = (markWidth * _numberOfStar);
    }
    ...
    if (_distributeSpacingEvenly) {
        [starMaskLayer setFrame:(CGRect){frameOffsetX, frameOffsetY, CGRectGetWidth(self.frame), _markImage.size.height}];
    } else {
...
- (CALayer *)highlightLayer
{
    ...
    if (_distributeSpacingEvenly) {
        highlightLayer.bounds = (CGRect){CGPointZero, CGRectGetWidth(self.frame), _markImage.size.height};
    } else {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_distributeSpacingEvenly) {
        ...
        totalStartsWidth = (_markImage.size.width * _numberOfStar);
    }
    ...
    if (CGRectContainsPoint(CGRectMake(frameOffsetX - _markImage.size.width, frameOffsetY, totalStartsWidth + _markImage.size.width, _markImage.size.height), location)) {
        ...
        float value;
        if (_distributeSpacingEvenly) {
            value = ((location.x - frameOffsetX)/ CGRectGetWidth(self.frame)) * _numberOfStar;
        } else {
            value = ((location.x - frameOffsetX)/ (_markImage.size.width * _numberOfStar)) * _numberOfStar;
        }
